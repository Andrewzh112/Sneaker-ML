...
bad_links = [
    "//stockx-assets.imgix.net/svg/icons/list-gray.svg?auto=compress,format",
...
def image_links(html, hype=True):
    ...
    for link in im_links:
        try:
            if hype:
                pair_links[link["alt"].split("alt=")[-1]] = link["src"].split("src=")[
                    -1
                ]
            else:
...
def get_stockx_html(start_url, page_num, amazon=False):
    ...
    driver.get(url)
    time.sleep(20)
    html = driver.execute_script("return document.documentElement.innerHTML;").encode(
        "utf-8"
    )
    driver.close()
    return html
