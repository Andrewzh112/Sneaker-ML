...
def train_loop(model, criterion, optimizer, train_loader, test_loader, epochs, device, save_model=False):
    ...
    for ep in range(epochs):
        ...
        print(f'Epoch {ep+1}/{epochs}, Train Loss:{train_mean_batch_loss:.4f}, \
        ...
        if save_model:
          if test_mean_batch_loss < best_loss:
            ...
            print(f'saving new best model with Test Loss: {test_mean_batch_loss:.4f}')
...
def objective(trial):
    ...
    train_loader, test_loader = get_dataloaders(os.path.join('/content','brand_data'),batch_size=BATCHSIZE)
...
def optimized_loop(model,optimizer):
    ...
    for epoch in range(EPOCHS):
        ...
        if accuracy > best_test:
          best_test = accuracy
          torch.save(model, f'/content/models/{model.__class__.__name__}_optimized.pt')
