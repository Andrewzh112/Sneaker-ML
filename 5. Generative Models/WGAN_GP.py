...
class WGANGP:
    def __init__(self):
        ...
        partial_gp_loss = partial(
            self.gradient_penalty_loss, averaged_samples=interpolated_img
        )
    ...
    def train(self, epochs, batch_size, sample_interval=50):
        ...
        X_train = np.stack([
            np.asarray(Image.open(img)) for img in glob(os.path.join("..","data","all_hyped", "**"))
        ]) / 127.5 - 1.0
        self.act1 = inception.predict(X_train)
        self.mu1, self.sigma1 = self.act1.mean(axis=0), cov(self.act1, rowvar=False)
        fid_distances = []
        # Adversarial ground truths
        valid = -np.ones(
            (batch_size, 1)
        )  # can try soft labels here as suggested by some papers
        ...
        for epoch in range(epochs):
            ...
            if epoch % 100 == 0:
                ...
                fid = self.calculate_fid(inception, gen_imgs)
    ...
    def calculate_fid(self, model, images2):
	# credit to https://machinelearningmastery.com/how-to-implement-the-frechet-inception-distance-fid-from-scratch/
...
if __name__=="__main__":
    wgan = WGANGP()
    fid_distances = wgan.train(epochs=30000, batch_size=32, sample_interval=10)
    plt.figure(figsize=(12,10))
    plt.plot(range(0,30000,100), fid_distances)
    plt.title('Frechet Inception Distance by Epochs')
    plt.xlabel('Epochs')
    plt.ylabel('FID')
    plt.show()